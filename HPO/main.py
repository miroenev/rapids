...
def parse_args():
    ...
    parser.add_argument('--coil_type', default='helix', type=str, help='the type of coil to generate the data')
    parser.add_argument('--num_blobs', default=1000, type=int, help='the number of blobs generated on the GPU')
    parser.add_argument('--num_coordinates', default=400, type=int, help='the number of starting locations of each blob')
    parser.add_argument('--sdev_scale', default=.3, type=float, help='standard deviation of normals used to generate data')
    parser.add_argument('--noise_scale', default=.1, type=float, help='additional noise')
    parser.add_argument('--coil_density', default=12.0, type=float, help='how tight the coils are')
    
    # ETL arguments
    parser.add_argument('--train_test_overlap', default=.05, help='percentage of train and test distribution that overlaps')
    
    # HPO arguments
    parser.add_argument('--num_gpus', default=1, type=int, help='the number of gpus to use')
    parser.add_argument('--num_timesteps', default=10, type=int, help='the number of timesteps to run HPO')
...
def main(args):
    ...
    # split
    trainData_cDF, trainLabels_cDF, testData_cDF, testLabels_cDF, t_split = rl.split_train_test_nfolds ( data, labels, trainTestOverlap = args.train_test_overlap)
